<nav class="navbar navbar-expand-lg navbar-dark green darken-1 menu-usuario py-0">
    <div class="collapse navbar-collapse barraPrincipal py-2">
        <ul class="navbar-nav">
            <!--Pagina Inicial-->
            <li class="nav-item">
                <a class="nav-link" href="../reitoria/index.jsp">
                    <img id="icone" src="../img/Icons/icones/png/home-1.png" title="Página Inicial">Página Inicial
                </a>
            </li>
            <!--Pefil do Usuário-->
            <li class="nav-item">
                <a class="nav-link" href="../errors/manutencao.jsp">
                    <img id="icone" src="../img/Icons/icones/png/folder-14.png" title="Caixa de Mensagens">Caixa de Mensagens
                </a>
            </li>
            <!--Oportunidade de Estágios-->
            <li class="nav-item">
                <a class="nav-link" href="../errors/manutencao.jsp">
                    <img id="icone" src="../img/Icons/icones/png/briefcase.png" title="Estágios/Trabalhos">Estágios/Trabalhos
                </a>
            </li>
            <!--Cursos Extra-curriculares-->
            <li class="nav-item">
                <a class="nav-link" href="../errors/manutencao.jsp">
                    <img id="icone" src="../img/Icons/icones/png/notepad-2.png" title="Cursos Extras">Cursos Extras
                </a>
            </li>
            <!--Manter Eventos-->
            <li class="nav-item">
                <a class="nav-link" href="../reitoria/eventos.jsp">
                    <img id="icone" src="../img/Icons/icones/png/newspaper.png" title="Eventos">Eventos
                </a>
            </li>
            <!--Pesquisar egressos-->
            <li class="nav-item">
                <a class="nav-link" href="../reitoria/manterEgresso.jsp">
                    <img id="icone" src="../img/student.png" alt="Egressos" title="Egressos">Egressos
                </a>
            </li>
            <!--Pesquisar campus-->
            <li class="nav-item">
                <a class="nav-link" href="../reitoria/manterCampus.jsp">
                    <img id="icone" src="../img/Icons/icones/png/campus.png" alt="Campus" title="Campus">Campus
                </a>
            </li>

            <!--Voltar para Página de Login-->
            <li class="nav-item">
                <a class="nav-link" href="../LoginServlet">
                    <img id="icone" src="../img/Icons/icones/png/exit-1.png" alt="Sair" title="Encerrar sessão">Encerrar sessão
                </a>
            </li>
        </ul>
    </div>
</nav>
<div class="green darken-2 white-text mb-4">
    <h6 class="ml-4 pt-2">Logado como Reitoria</h6>
</div>
